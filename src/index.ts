#!/usr/bin/env node
/**
 * Fireblocks MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Client sends their Fireblocks credentials as a Bearer token.
 * The token is a base64-encoded JSON object:
 *   {"apiKey":"...","apiSecret":"...","env":"sandbox|production"}
...
app.get('/', (_req, res) => {
  ...
  res.json({
    ...
    version: '1.0.0',
    description:
      'Institutional crypto custody — vaults, transactions, staking, tokenization, compliance, webhooks via MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass a base64-encoded JSON credential object as the Bearer token. No credentials are stored on this server.',
      header: 'Authorization: Bearer <base64-json>',
      tokenFormat: '{"apiKey":"YOUR_API_KEY","apiSecret":"YOUR_RSA_PRIVATE_KEY_PEM","env":"sandbox|production"}',
      howToGetKey:
        'Generate API credentials at https://console.fireblocks.io → Settings → API Users → Add API User',
    },
    configTemplate: {
      mcpServers: {
        fireblocks: {
...
<head>
  ...
  <title>Fireblocks MCP Server — AgenticLedger</title>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    input[type="text"],textarea{width:100%;padding:0.6rem 0.75rem;font-size:0.95rem;font-family:inherit;border:1px solid #cbd5e1;border-radius:8px;outline:none;transition:border-color .15s}
    input[type="text"]:focus,textarea:focus{border-color:#2563EB;box-shadow:0 0 0 3px rgba(37,99,235,.12)}
    textarea{min-height:80px;font-family:'Fira Mono','Consolas',monospace;font-size:0.85rem;resize:vertical}
    select{padding:0.6rem 0.75rem;font-size:0.95rem;font-family:inherit;border:1px solid #cbd5e1;border-radius:8px;outline:none;width:100%}
...
<body>
  ...
  <div class="container">
    <h1>Fireblocks MCP Server</h1>
    <p class="subtitle">Institutional crypto custody — vaults, transactions, staking, tokenization, compliance, webhooks. ${tools.length} tools available.</p>

    <div class="card">
      <h2>How to get your Fireblocks API credentials</h2>
      <ol class="steps">
        <li>Log in to the <a href="https://console.fireblocks.io" target="_blank" rel="noopener">Fireblocks Console</a>.</li>
        <li>Go to <strong>Settings &rarr; API Users</strong>.</li>
        <li>Click <strong>Add API User</strong> and download the RSA private key.</li>
        <li>Copy your <strong>API Key</strong> from the console.</li>
        <li>Paste both values below to generate your MCP config.</li>
      </ol>
    </div>
    ...
    <div class="card">
      ...
      <div class="field-row">
        <label for="apiKey">Fireblocks API Key</label>
        <input type="text" id="apiKey" placeholder="Paste your API key here..." autocomplete="off" spellcheck="false" />
      </div>
      <div class="field-row">
        <label for="apiSecret">RSA Private Key (PEM)</label>
        <textarea id="apiSecret" placeholder="-----BEGIN RSA PRIVATE KEY-----&#10;...&#10;-----END RSA PRIVATE KEY-----" spellcheck="false"></textarea>
      </div>
      <div class="field-row">
  ...
  <script>
    ...
    function buildConfig() {
      ...
      let bearerToken = 'YOUR_BASE64_TOKEN';

      if (key && secret) {
        const creds = JSON.stringify({ apiKey: key, apiSecret: secret, env: env });
        bearerToken = btoa(creds);
      }
    ...
    function update() { configBlock.textContent = buildConfig(); }

    apiKeyInput.addEventListener('input', update);
    apiSecretInput.addEventListener('input', update);
...
function extractCredentials(req: express.Request): { apiKey: string; apiSecret: string; env: string } | null {
  ...
  try {
    ...
    if (creds.apiKey && creds.apiSecret) {
      // Handle escaped newlines in the PEM key
      const apiSecret = creds.apiSecret.replace(/\\n/g, '\n');
      return {
        apiKey: creds.apiKey,
        apiSecret,
...
function createMCPServer(client: FireblocksClient): Server {
  const server = new Server(
    { name: 'fireblocks-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
...
app.post('/mcp', async (req, res) => {
  ...
  if (!creds) {
    res.status(401).json({
      error: 'Missing or invalid Authorization header. Use: Bearer <base64-encoded-json>',
      tokenFormat: '{"apiKey":"...","apiSecret":"...","env":"sandbox|production"}',
      hint: 'Base64-encode the JSON object and pass as Bearer token.',
    });
    return;
  }

  // Create per-session API client with the user's credentials
  const client = new FireblocksClient(creds.apiKey, creds.apiSecret, creds.env);
...
<head>
  ...
  <title>Authorize \u2014 Fireblocks MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>Fireblocks MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>Fireblocks MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`Fireblocks MCP HTTP Server running on port ${PORT}`);
  console.log(`  MCP endpoint:   http://localhost:${PORT}/mcp`);
  console.log(`  Health check:   http://localhost:${PORT}/health`);
  console.log(`  Tools:          ${tools.length}`);
  console.log(`  Transport:      Streamable HTTP`);
  console.log(`  Auth:           Bearer passthrough (base64-encoded Fireblocks credentials)`);
});
